...
export default function OnTheAir() {
    ...
    const fetchOnTheAir = () => {
        ...
        if (JSON.stringify(initialFilter) != JSON.stringify(filter)) {
            apiUrl = `https://api.themoviedb.org/3/tv/on_the_air?language=en-US&page=${page}&${queryString}`;
        } else {
            apiUrl = `https://api.themoviedb.org/3/tv/on_the_air?language=en-US&page=${page}&popularity.desc`;
        }

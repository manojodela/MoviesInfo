...
export default function PopularTvShows() {
    ...
    const fetchTvShows = () => {
        ...
        if (JSON.stringify(initialFilter) != JSON.stringify(filter)) {
            apiUrl = `https://api.themoviedb.org/3/discover/tv?include_adult=false&page=${page}&${queryString}`;
        } else {
            apiUrl = `https://api.themoviedb.org/3/discover/tv?include_adult=false&language=en-US&page=${page}&sort_by=popularity.desc`;
        }

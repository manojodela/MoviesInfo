...
export default function TopRatedShows() {
    ...
    const fetchTopRated = () => {
        ...
        if (JSON.stringify(initialFilter) != JSON.stringify(filter)) {
            apiUrl = `https://api.themoviedb.org/3/tv/top_rated?language=en-US&page=${page}&${queryString}`;
        } else {
            apiUrl = `https://api.themoviedb.org/3/tv/top_rated?language=en-US&page=${page}&popularity.desc`;
        }

...
const initialFilter = {
    ...
    "vote_average.gte": "0",
    "vote_average.lte": "10",
    "release_date.gte": pastDate.format('YYYY-MM-DD'),
    "release_date.lte": currentDate.format('YYYY-MM-DD'),
    with_release_type: "3"
}
...
export default function NowPlayingMovies() {
    ...
    const fetchMovies = () => {
        ...
        if (JSON.stringify(initialFilter) != JSON.stringify(filter)) {
            ...
            apiUrl = `https://api.themoviedb.org/3/movie/now_playing?language=en-US&page=${page}&popularity.desc`;
        }

        fetch(apiUrl, options)
            .then(response => response.json())
            .then(response => { setMovies(response.results); setTotalCount(response.total_results); console.log(response); })
            ...
            {loading ? (
                ...
                <Layout>
                    ...
                    <div className="text-end my-2 p-3" >
                        <Pagination current={page} showQuickJumper defaultCurrent={1} total={totalCount} onChange={onChange} pageSize={20} />
                    </div>
                </Layout>
            }
        </>

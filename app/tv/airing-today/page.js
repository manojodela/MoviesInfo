...
// SSR: Airing today - render on every request for real-time TV schedule
// This content changes daily and users want the most current information
export const revalidate = 0; // SSR - no caching

export const metadata = {
  title: 'Airing Today TV Shows - MoviesInfo',
...
async function AiringTodayContent({ page = 1 }) {
  const tvData = await discoverTV({ page, sort_by: 'popularity.desc' });
  ...
  return (
    <>
      ...
      <Pagination currentPage={page} totalPages={totalPages} baseUrl="/tv/airing-today" />
    </>
  );
}

export default function AiringTodayPage({ searchParams }) {
  const page = parseInt(searchParams?.page) || 1;

  return (
    <div>
      <h1 style={{ marginBottom: '30px' }}>Airing Today</h1>
      <Suspense fallback={<div style={{ textAlign: 'center', padding: '40px', fontSize: '1.1rem', color: '#999' }}>Loading TV shows...</div>}>
        <AiringTodayContent page={page} />
      </Suspense>
    </div>
  );
}

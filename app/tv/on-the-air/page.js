...
async function OnTheAirContent({ page = 1 }) {
  ...
  return (
    <>
      ...
      <Pagination currentPage={page} totalPages={totalPages} baseUrl="/tv/on-the-air" />
    </>
  );
}

export default function OnTheAirPage({ searchParams }) {
  const page = parseInt(searchParams?.page) || 1;

  return (
    <div>
      <h1 style={{ marginBottom: '30px' }}>On The Air</h1>
      <Suspense fallback={<div style={{ textAlign: 'center', padding: '40px', fontSize: '1.1rem', color: '#999' }}>Loading TV shows...</div>}>
        <OnTheAirContent page={page} />
      </Suspense>
    </div>
  );
}
